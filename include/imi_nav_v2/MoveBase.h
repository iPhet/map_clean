...
private:
    ...
    void handleGo() {
      ...
      if(dDevYaw>0.0175){
        move(m_dGoSpeed,-0.2);
//        ROS_INFO("left deviation!d=%f,DevYaw=%f",d,dDevYaw);
      }else if(dDevYaw<-0.0175){
        move(m_dGoSpeed,0.2);
//        ROS_INFO("right deviation!d=%f,DevYaw=%f",d,dDevYaw);
      }else{
        move(m_dGoSpeed,0);
//        ROS_INFO("no deviation!d=%f,DevYaw=%f",d,dDevYaw);
      }
//      double A=tan(m_GoalPose.w);
//      double B=-1.0f;
//      double C=m_GoalPose.y-A*m_GoalPose.x;
...
//        ROS_INFO("goalPose=%f,currentPose=%f,deltaYaw=%f",m_GoalPose.w,m_currentPose.w,dDeltaYaw);

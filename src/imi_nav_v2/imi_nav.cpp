...
//      p1.x=pose.x+sin(pose.w)*0.2;
//      p1.y=pose.y+cos(pose.w)*0.2;
//      p2.x=pose.x-sin(pose.w)*0.2;
//      p2.y=pose.y-cos(pose.w)*0.2;
//      p3.x=pose.x+cos(pose.w)*0.5-sin(pose.w)*0.2;
//      p3.y=pose.y+sin(pose.w)*0.5-cos(pose.w)*0.2;
//      p4.x=pose.x+cos(pose.w)*0.5+sin(pose.w)*0.2;
//      p4.y=pose.y+sin(pose.w)*0.5+cos(pose.w)*0.2;
//      GMapping::IntPoint poseInMap1 = (*m_pSmap).world2map(GMapping::Point(p1.x,p1.y));
//      GMapping::IntPoint poseInMap2 = (*m_pSmap).world2map(GMapping::Point(p2.x,p2.y));
//      GMapping::IntPoint poseInMap3=(*m_pSmap).world2map(GMapping::Point(p3.x,p3.y));
//      GMapping::IntPoint poseInMap4=(*m_pSmap).world2map(GMapping::Point(p4.x,p4.y));
//      SPose np;
//      np.x=p1.x-pose.x;
//      np.y=p1.y-pose.y;
//      double dmin=10.0;
...
//      int frontYmin=0;
//      int frontYmax=0;
//      if(getMinMax(poseInMap1.x,poseInMap2.x,poseInMap3.x,poseInMap4.x,frontXmin,frontXmax)&&
//         getMinMax(poseInMap1.y,poseInMap2.y,poseInMap3.y,poseInMap4.y,frontYmin,frontYmax)){
//        for(int x=frontXmin;x<frontXmax;x++){
//          for(int y=frontYmin;y<frontYmax;y++){
//            if( m_map.data[MAP_IDX(m_map.info.width,x,y)]>0){
//              GMapping::Point wPose = (*m_pSmap).map2world(x,y);
//              SPose rPose;
//              if(transformWorld2Robot(wPose,rPose)){
//                if(isPointInRect(p1,p2,p3,p4,rPose)){
//                  m_vBlock.push_back(rPose);
//                  double d=rPose.x-sqrt(0.2*0.2-pow(fabs(rPose.y),2));
//                  if(d<dmin){
//                    dmin=d;
//                    m_NearestObstale=rPose;
//                  }
...
//              p.x=pw.x;
//              p.y=pw.y;
//              if(isPointInRect(p1,p2,p3,p4,p)){
//                m_bMapBlock=true;
//                m_vBlock.push_back(p);
//                SPose op;
//                op.x=p.x-pose.x;
//                op.y=p.y-pose.y;
//                double cos_theta=(np.x*op.x+np.y*op.y)/(sqrt(np.x*np.x+np.y*np.y)*sqrt(op.x*op.x+op.y*op.y));
//                double d=sqrt(pow(pose.x-p.x,2)+pow(pose.y-p.y,2));
//                double d1=d*cos_theta;
//                double d2=sqrt(d*d-d1*d1);
//                double di=d1-sqrt(0.2*0.2-d2*d2);
//                if(di<dmin){
//                  dmin=di;
//                  m_NearestObstale=p;
//                }
